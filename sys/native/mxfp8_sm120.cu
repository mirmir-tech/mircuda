...
namespace mircuda::mxfp8 {

using namespace cute;
using ElementType = cutlass::float_e4m3_t;
using ElementSF = cutlass::float_ue8m0_t;
using ElementA = cutlass::mx_float8_t<ElementType>;
using ElementB = cutlass::mx_float8_t<ElementType>;
...
auto arguments(const Plan& plan, const void* a, const void* a_scales,
               const void* b, const void* b_scales, void* c) {
  ...
  return Gemm::Arguments{
      ...
      {{1.0f, 0.0f}, static_cast<const ElementC*>(c), stride_c,
...
int prepare(Plan* plan) {
  ...
  if (plan->workspace_bytes > 0) {
    const int allocation = static_cast<int>(cudaMallocAsync(
...
extern "C" int mircuda_mxfp8_tc_create(int m, int n, int k, void* stream,
                                         void** output) {
  using namespace mircuda::mxfp8;
  if (m <= 0 || n <= 0 || k <= 0 || k % 128 != 0 || stream == nullptr ||
...
extern "C" size_t mircuda_mxfp8_tc_workspace_bytes(const void* raw) {
  const auto* plan = static_cast<const mircuda::mxfp8::Plan*>(raw);
  return plan == nullptr ? 0 : plan->workspace_bytes;
}

extern "C" int mircuda_mxfp8_tc_execute(
  ...
  auto args = arguments(*plan, a, a_scales, b, b_scales, c);
...
extern "C" void mircuda_mxfp8_tc_destroy(void* raw) {
  auto* plan = static_cast<mircuda::mxfp8::Plan*>(raw);
