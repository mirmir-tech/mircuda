...
unsafe extern "C" {
    ...
    fn mircuda_mxfp8_tc_workspace_bytes(plan: *const c_void) -> usize;
    fn mircuda_mxfp8_tc_execute(
        plan: *mut c_void,
        stream: *mut c_void,
        a: *const c_void,
    ...
    fn mircuda_mxfp8_tc_destroy(plan: *mut c_void);
}
...
// SAFETY: the plan retains its CUDA stream and execution requires exclusive access.
unsafe impl Send for BlockScaledMxFp8Plan {}

impl Context {
    pub fn create_block_scaled_mxfp8_plan(
        &self,
        stream: &Stream,
        spec: BlockScaledMxFp8Spec,
...
impl BlockScaledMxFp8Plan {
    ...
    pub fn workspace_bytes(&self) -> usize {
        ...
        unsafe { mircuda_mxfp8_tc_workspace_bytes(self.raw.as_ptr()) }
    }
...
fn validate_sizes(
    ...
    let valid = spec.m.checked_mul(spec.k) == Some(a.bytes())
        && scale_bytes(spec.m, spec.k) == Some(a_scales.bytes())
        && spec.n.checked_mul(spec.k) == Some(b.bytes())
...
fn scale_bytes(rows: usize, columns: usize) -> Option<usize> {
    rows.div_ceil(128).checked_mul(128)?.checked_mul((columns / 32).div_ceil(4) * 4)
}
