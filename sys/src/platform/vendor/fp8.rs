...
unsafe extern "C" {
    fn mircuda_cublaslt_fp8_create(
        m: i32,
        n: i32,
        k: i32,
        stream: *mut c_void,
    ...
    fn mircuda_cublaslt_fp8_execute(
        plan: *mut c_void,
        stream: *mut c_void,
        a: *const c_void,
        b: *const c_void,
    ...
    fn mircuda_cublaslt_fp8_destroy(plan: *mut c_void);
}
...
unsafe impl Send for CublasLtFp8Plan {}

impl Context {
    pub fn create_cublaslt_fp8_plan(
        &self,
        stream: &Stream,
        spec: CublasLtFp8Spec,
...
fn validate_sizes(
    ...
    let valid = checked_bytes(spec.m, spec.k, 1) == Some(a.bytes())
        && checked_bytes(spec.n, spec.k, 1) == Some(b.bytes())
        && a_scale.bytes() == size_of::<f32>()
        && b_scale.bytes() == size_of::<f32>()
        && checked_bytes(spec.m, spec.n, size_of::<u16>()) == Some(c.bytes());
    if valid {
        Ok(())
    } else {
